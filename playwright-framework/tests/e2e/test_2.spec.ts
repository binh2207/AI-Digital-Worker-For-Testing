...
test.describe('TEST-2 — Select variants and add products to cart', () => {
  ...
  test('TC-05a: Cart page lists added items with correct name, price and Continue Shopping link', async ({ page }) => {
    const cartPage = new SelectVariantsAndAddProductsToCartPage(page);
    await cartPage.goto();
    await cartPage.clickAddToCart();
  ...
  test('TC-05b: Empty cart shows empty-state message and a functional Continue Shopping link', async ({ page }) => {
    const cartPage = new SelectVariantsAndAddProductsToCartPage(page);
    await cartPage.gotoCart();
